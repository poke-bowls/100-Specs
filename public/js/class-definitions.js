...
Person.prototype.spendMoney = function ( moolah ) {
    this.money -= moolah;
  };
Person.prototype.earnMoney = function ( guap ) {
    this.money += guap;
  };
...
PrincessLeia.prototype.marries = function( interest ) {
  if ( interest === "Luke Skywalker" ) {
    return "Gross!";
  } else if ( interest !== "Han Solo" ) {
    return false;
  } else {
  return true;
  }
...
function Scientist( name, money, age, gender, disciplines, properties ) {
  Person.call( this, name, money, age, gender );
  this.disciplines = [];
  this.discoveries = [];
...
Scientist.prototype.addDiscovery = function( string ) {
  ...
  if ( this.discoveries.length > 2 ) {
    var first = this.discoveries[0];
    var second = this.discoveries[1];
    var third = this.discoveries[2];
  return "I discovered " + first + ", " + second + ", and " + third  + ".";
